<template>
  <div>
    <div style="display: flex; justify-content: space-between">
      <el-button icon="el-icon-plus" type="primary" @click="showAddSalaryView">添加工资</el-button>
    ...
    <div style="margin-top: 10px">
      <el-table :data="salaries" border stripe>
        ...
        <el-table-column width="120" prop="employeeName" label="员工姓名"></el-table-column>
...
export default {
  ...
  data() {
    return {
      ...
      departmentId: 1 // 假设部门ID为1，可以根据实际情况动态获取
    }
  },
  mounted() {
    this.initSalaries();
  ...
  methods: {
    showEditSalaryView(data) {
      this.dialogTitle = '修改工资';
      this.dialogVisible = true;
      this.salary.employeeName = data.employeeName;
    ...
    deleteSalary(data) {
      this.$confirm('此操作将删除【' + data.employeeName + '】的工资信息，是否继续？', '提示', {
        cancelButtonText: '取消',
    ...
    showAddSalaryView() {
      ...
      this.dialogTitle = '添加工资';
      this.activeItemIndex = 0;
      this.dialogVisible = true;
    },
    initSalaries() {
      this.getRequest("/salary/sob/department?departmentId=" + this.departmentId).then(resp => {
        if (resp) {
          this.salaries = resp;
        }
      })
